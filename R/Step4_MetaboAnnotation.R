...
MetaboAnnotation <- function(AnnotaData, masstole = 10, toleUnit = 1, annotaDB = "metlin", ionMode  = "pos"){
  ...
  ####
  #load("D:/Code/Rpackage/mmeaseR3/data/annotation_data.RData")

  annotation_data <- function() {
    utils::data(list="annotation_data", package="mmeaseR")
    ...
    for (addIdx in 1:length(param$charge)){
      ...
      for (mz in param$mzlist){
        ...
        if(db=='metlin'){
          ...
          queryLines <- MS_database[which(MS_database$MetlinMass>=minMz & MS_database$MetlinMass<=maxMz),c("MMEASE_ID", "MetlinMass", "MID", "Common_name", "Annotation")]

        }else{
          queryLines <- MS_database[which(MS_database$MonoisotopicMass>=minMz & MS_database$MonoisotopicMass<=maxMz & MS_database$Source==param$annotaDB),c("MMEASE_ID", "MonoisotopicMass", "MID", "Common_name", "Annotation")]
...
AnnotationTandom <- function(AnnotaParamTandom){
  ...
  #
  #load("D:/Code/Rpackage/mmeaseR3/data/annotationTandom_data.RData")


  annotationTandom_data <- function() {
    utils::data(list="annotationTandom_data", package="mmeaseR")
    get("annotationTandom_data", envir = .GlobalEnv)
  }

  annotationTandom_data <- annotationTandom_data()


  Annotation_db <- annotationTandom_data$Annotation_db
  options(digits=2)
  AnnotationTandom <- Annotation_db[which(Annotation_db$Mass>=minMzT & Annotation_db$Mass<=maxMzT & Annotation_db$Energy == AnnotaParamTandom$ionEnergy & Annotation_db$Ionization == AnnotaParamTandom$ModeTandom),c("MMEASE_ID", "Label", "Spectrum", "Mass", "Name", "Annotation")]
...
annotaTandom_plot <- function(AnnotaParamTandom, TandomData){
  ...
  plot_cand <- AnnotationTables$MMEASE_ID
  ...
  mSpectrum <- AnnotationTables[which(AnnotationTables[,"MMEASE_ID"]==MSMSid),"Spectrum"]
...
AnnotaData <- function() {
  utils::data(list="AnnotaData", package="mmeaseR")
